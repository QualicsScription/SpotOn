...
logging.basicConfig(filename='file_comparator.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                return {
                    ...
                    'raw_data': {'feature_tree': feature_data, 'sketch_data': sketch_data, 'geometry': geometry_data}
                }
        except Exception as e:
            logging.error(f"SolidWorks dosya parsing hatasi: {e}")
            return {'features': [], 'sketches': [], 'geometry_stats': {}, 'raw_data': {'feature_tree': b'', 'sketch_data': b'', 'geometry': b''}}
            ...
            while i < len(data):
                if 32 < data[i] < 127:
                    ...
                    while i < len(data) and 32 < data[i] < 127:
                        i += 1
                    if i - start > 3:
                        features.append({'name': data[start:i].decode('ascii', errors='ignore'), 'offset': start, 'params': {}})
                i += 1
            return features
        except Exception as e:
            logging.error(f"Feature cıkarma hatasi: {e}")
            return []
        ...
        try:
            ...
            for marker in markers:
                ...
                while True:
                    ...
                    if pos == -1:
                        break
                    sketches.append({'type': marker.decode('ascii'), 'offset': pos, 'data': data[pos:pos+20]})
                    pos += len(marker)
            return sketches
        except Exception as e:
            logging.error(f"Sketch cıkarma hatasi: {e}")
            return []

    def extract_geometry_stats(self, data):
        try:
            stats = {'signature': hashlib.md5(data).digest(), 'data_size': len(data), 'volume': 1.0}
            ...
            logging.error(f"Geometri istatistikleri cıkarma hatasi: {e}")
            return {'signature': b'', 'data_size': 0, 'volume': 1.0}

class SolidWorksAnalyzer:
    def __init__(self):
        self.parser = SWFileParser()
        self.weights = {'feature_tree': 0.5, 'sketch_data': 0.3, 'geometry': 0.2, 'metadata': 0.1}

    def read_binary_chunk(self, file_path, offset, size):
        try:
            with open(file_path, 'rb') as f:
                f.seek(offset if offset >= 0 else offset, os.SEEK_END if offset < 0 else os.SEEK_SET)
                return f.read(size)
        except Exception as e:
            logging.error(f"Binary chunk okuma hatasi: {e}")
            return b''

    def compare_feature_tree(self, file1, file2):
        return difflib.SequenceMatcher(None, self.read_binary_chunk(file1, 0x1000, 500), self.read_binary_chunk(file2, 0x1000, 500)).ratio() * 100
    ...
    def compare_sw_features(self, features1, features2):
        ...
        param_sim = sum(1 for f1 in features1 for f2 in features2 if f1['name'] == f2['name']) / max(len(features1), len(features2)) if features1 and features2 else 0.0
    ...
    def compare_sketches(self, sketches1, sketches2):
        ...
        count1, count2 = Counter(types1), Counter(types2)
        all_types = set(count1.keys()) | set(count2.keys())
        return sum(min(count1.get(t, 0), count2.get(t, 0)) for t in all_types) / sum(max(count1.get(t, 0), count2.get(t, 0)) for t in all_types) * 100 if all_types else 0.0

    def compare_geometry(self, geom1, geom2):
        if not geom1 or not geom2:
            return 0.0
        size_sim = 1.0 - abs(geom1['volume'] - geom2['volume']) / max(geom1['volume'], geom2['volume']) if 'volume' in geom1 and 'volume' in geom2 and geom1['volume'] > 0 and geom2['volume'] > 0 else 0.0
        sig_sim = difflib.SequenceMatcher(None, geom1['signature'], geom2['signature']).ratio() if 'signature' in geom1 and 'signature' in geom2 else 0.0
        return (size_sim * 0.6 + sig_sim * 0.4) * 100

    def compare(self, file1, file2):
        try:
            data1, data2 = self.parser.parse_features(file1), self.parser.parse_features(file2)
            binary_similarity = self.compare_feature_tree(file1, file2)
            if binary_similarity > 99.5:
                return {'score': 100.0, 'details': {'feature_tree': 100.0, 'sketch_data': 100.0, 'geometry': 100.0}, 'size_similarity': 100.0, 'match': True, 'type': 'solidworks'}
            ...
            raw_comparisons = {key: difflib.SequenceMatcher(None, data1['raw_data'][key], data2['raw_data'][key]).ratio() * 100 for key in data1['raw_data']}
            size1, size2 = os.path.getsize(file1), os.path.getsize(file2)
            size_similarity = min(size1, size2) / max(size1, size2) * 100 if max(size1, size2) > 0 else 0
            detailed_results = {'feature_tree': feature_similarity, 'sketch_data': sketch_similarity, 'geometry': geometry_similarity}
            total_score = (feature_similarity * self.weights['feature_tree'] + sketch_similarity * self.weights['sketch_data'] + geometry_similarity * self.weights['geometry']) / sum(self.weights.values())
            ...
            return {'score': final_score, 'details': detailed_results, 'raw_comparisons': raw_comparisons, 'size_similarity': size_similarity, 'match': final_score > 98, 'type': 'solidworks'}
        except Exception as e:
            logging.error(f"SolidWorks karsilastirma hatasi: {e}")
...
class GeneralComparator:
    def compare(self, file1, file2):
        try:
            ...
            size_similarity = (1 - abs(stat1.st_size - stat2.st_size) / max(stat1.st_size, stat2.st_size)) * 100 if max(stat1.st_size, stat2.st_size) > 0 else 0
            time_similarity = max(0, 100 - (abs(stat1.st_mtime - stat2.st_mtime) / 86400 * 100)) if abs(stat1.st_mtime - stat2.st_mtime) < 86400 else 0
            content_similarity = 0
            try:
                with open(file1, 'rb') as f1, open(file2, 'rb') as f2:
                    header1, header2 = f1.read(1024), f2.read(1024)
                    header_similarity = difflib.SequenceMatcher(None, header1, header2).ratio() * 100
                    f1.seek(stat1.st_size // 2)
                    f2.seek(stat2.st_size // 2)
                    mid_similarity = difflib.SequenceMatcher(None, f1.read(1024), f2.read(1024)).ratio() * 100
            ...
            if size_similarity > 99:
                try:
                    hash1, hash2 = hashlib.md5(open(file1, 'rb').read()).hexdigest(), hashlib.md5(open(file2, 'rb').read()).hexdigest()
                    hash_match = hash1 == hash2
                except:
                    pass
            total_score = size_similarity * 0.3 + time_similarity * 0.2 + content_similarity * 0.5
            return {'score': total_score, 'size_similarity': size_similarity, 'time_similarity': time_similarity, 'content_similarity': content_similarity, 'match': hash_match, 'type': 'general'}
        except Exception as e:
            logging.error(f"Genel karsilastirma hatasi: {e}")
...
class FileComparator:
    ...
    def detect_manipulation(self, file1, file2, comparison_results):
        try:
            ...
            weights = {'size_ratio': 0.2, 'time_diff': 0.3, 'content_injection': 0.3, 'rename_pattern': 0.2}
            ...
            return {'detected': manipulation_score > 0.7, 'score': manipulation_score * 100, 'type': manipulation_type, 'indicators': indicators}
        except Exception as e:
            logging.error(f"Manipulasyon tespit hatasi: {e}")
            return {'detected': False, 'score': 0, 'type': 'none', 'indicators': {}}

    def classify_result(self, score, hash_match, file_type):
        if file_type == 'solidworks':
            if hash_match or score >= 98: return "Tam Eslesme"
            elif score >= 85: return "Save As Kopyasi"
            elif score >= 70: return "Kucuk Degisiklikler"
            elif score >= 40: return "Buyuk Degisiklikler"
            else: return "Farkli Dosyalar"
        else:
            if hash_match or score >= 95: return "Tam Eslesme"
            elif score >= 80: return "Cok Benzer"
            elif score >= 60: return "Orta Benzerlik"
            elif score >= 30: return "Zayif Benzerlik"
            else: return "Farkli Dosyalar"
    ...
    def compare_files(self, file1, file2):
        try:
            ...
            comparison_result = {'file1': file1, 'file2': file2, 'total': result['score'], 'category': category, 'manipulation': manipulation, 'file_type': file_type, 'match': result.get('match', False)}
            if file_type == 'solidworks':
                comparison_result.update({
                    ...
                    'details': {'feature_tree': result.get('feature_tree', 0), 'sketch_data': result.get('sketch_data', 0), 'geometry': result.get('geometry', 0)}
                })
            else:
                comparison_result.update({
            ...
            logging.error(f"Dosya karsilastirma hatasi: {e}")
            return {'file1': file1, 'file2': file2, 'metadata': 0, 'hash': 0, 'content': 0, 'structure': 0, 'total': 0, 'category': "Hata", 'manipulation': {'detected': False}, 'file_type': 'unknown', 'match': False, 'error': str(e)}